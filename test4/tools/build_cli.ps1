# ============================================================================
#  build_cli.ps1 - build test4 WITHOUT ninja/cmake.
#
...
#  equivalent. Result: build\cli\test4.elf
#
#  Usage:
...
$out    = Join-Path $outdir "test4.elf"
...
$appNames = @("main","gpio","freertos","adc","tim","usart",
...
$halNames = @("stm32f1xx_hal_gpio_ex","stm32f1xx_hal_tim","stm32f1xx_hal_tim_ex",
              "stm32f1xx_hal_adc","stm32f1xx_hal_adc_ex","stm32f1xx_hal",
              "stm32f1xx_hal_rcc","stm32f1xx_hal_rcc_ex","stm32f1xx_hal_gpio",
              "stm32f1xx_hal_dma","stm32f1xx_hal_cortex","stm32f1xx_hal_pwr",
...
$myNames  = @("app","motor","encoder","adc_knob","pid","vofa")
...
$flags = @(
  ...
  "--specs=nano.specs",
  "-Wl,--gc-sections",
  ("-Wl,-Map=" + (Join-Path $outdir "test4.map")),
  "-Wl,--print-memory-usage"
)

Write-Host ("Compiling+linking " + $srcs.Count + " files ...") -ForegroundColor Cyan
$sw = [System.Diagnostics.Stopwatch]::StartNew()
$log = & $gcc @flags @inc @srcs -o $out 2>&1
$rc = $LASTEXITCODE
$sw.Stop()
$log | Where-Object { $_ -match "error|warning:|RAM:|FLASH:" } | Select-Object -First 20
...
& $objc -O binary $out (Join-Path $outdir "test4.bin")
