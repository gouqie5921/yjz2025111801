...
/* 进入陀螺仪模式时以当时姿态为零位，云台回中，然后按下面增益跟随。
   增益用 1.0（1:1 跟手）；行程不够的问题由"软饱和"（零位跟着走）解决，
   而不是靠降低增益——降增益会让手感变成"转大角度云台才转小角度"。 */
#define APP_GYRO_CENTER         90.0f   /* 云台中位角度 */
#define APP_GYRO_PAN_GAIN       1.0f    /* 水平跟随比例 1:1：手上转多少，云台跟多少 */
/* 俯仰增益取 2.0（实测调参结果）：云台指令行程固定 ±65°，增益 2.0 时
   "手抬约 32° 就走完全程"。调参过程：1.0 要抬 180° 才有明显动作（太大）
   → 1.5 约 43° → 3.0 约 22°（抖动放大明显）→ 最终定在 2.0。
   软饱和边界自动按 APP_GYRO_RANGE/增益 计算，只改这个宏即可。 */
#define APP_GYRO_TILT_GAIN      2.0f    /* 俯仰跟随比例 */
#define APP_GYRO_RANGE          65.0f   /* 云台可用半行程：从机指令余量 ±25° → 90-25=65 */
#define APP_GYRO_LIMIT          APP_GYRO_RANGE  /* 兼容旧名；输出限幅即软饱和边界 */
